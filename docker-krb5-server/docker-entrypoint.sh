...
if [ ! -f "/var/lib/krb5kdc/principal" ]; then
    ...
    if [ -z ${KRB5_USER} ]; then
        echo "No Username provided will use 'joedoe'."
        KRB5_USER="joedoe"
    fi

    if [ -z ${KRB5_HTTP_SVC} ]; then
        echo "No HTTP service principal name provided using 's2.example.com'."
        KRB5_HTTP_SVC="s2.example.com"
    fi

    if [ -z ${KRB5_USER_PASS} ]; then
        echo "No User Password for kdb provided; Creating one now."
        KRB5_USER_PASS=`< /dev/urandom tr -dc _A-Z-a-z-0-9 | head -c${1:-32};echo;`
        echo "Using User Password ${KRB5_USER_PASS}"
    fi
...
EOT
    ...
    echo "Creating user principal ${KRB5_USER}"
    kadmin.local -q "addprinc -pw ${KRB5_USER_PASS} ${KRB5_USER}"


    echo "Creating service principal ${KRB5_HTTP_SVC}"
    kadmin.local -q "addprinc -randkey HTTP/${KRB5_HTTP_SVC}@${KRB5_REALM}"
    echo "Exporting keytab "
    kadmin.local -q "ktadd -norandkey -k  /var/lib/krb5kdc/${KRB5_HTTP_SVC}.keytab HTTP/${KRB5_HTTP_SVC}@${KRB5_REALM}"
    # Changing to world readable since we want keycloak to be able to read this from the same volume
    # DO NOT DO THIS FOR A PRODUCTION SYSTEM
    chmod 644 /var/lib/krb5kdc/${KRB5_HTTP_SVC}.keytab
fi
